            ...
            <div class="col-12 col-md-8 col-lg-6 col-xxl-4 py-8 py-xl-0">
                ...
                <div class="card smooth-shadow-md">
                    ...
                    <div class="card-body p-6">
                        ...
                        <!-- Form -->
                        <form method="POST" action="{{ route('update_mahasiswa',['id' => $user->id ]) }}">
                            @csrf
                            @method('PUT')
                            <!-- Username -->
                            <div class="mb-3">
                                ...
                                <input value="{{ $user->name }}" type="text" id="name" class="form-control"
                                    name="name" placeholder="name address here" required="">
                            </div>
                            <div class="mb-3">
                                <label for="username" class="form-label">username</label>
                                <input value="{{ $user->username }}" type="text" id="username" class="form-control"
                                    name="username" placeholder="username here" required="">
                            </div>
                            <div class="mb-3">
                                <label for="email" class="form-label">email</label>
                                <input value="{{ $user->email }}" type="email" id="email" class="form-control"
                                    name="email" placeholder="email here" required="">
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <input value="{{ $user->mahasiswa->nim }}" type="number" id="nim"
                                    class="form-control" name="nim" placeholder="nim address here" required="">
                            </div>
                            <div class="mb-3">
                                <label for="jurusan" class="form-label">Jurusan</label>
                                <select name="jurusan" id="jurusan" class="form-select"
                                    aria-label="Default select example" onchange="updateKelasOptions()">
                                    <option value="TI" {{ 'TI' == $user->mahasiswa->jurusan ? 'selected' : '' }}>TI
                                    </option>
                                    <option value="TPTU" {{ 'TPTU' == $user->mahasiswa->jurusan ? 'selected' : '' }}>TPTU
                                    </option>
                                    <option value="TM" {{ 'TM' == $user->mahasiswa->jurusan ? 'selected' : '' }}>TM
                                    </option>
                                    <option value="KP" {{ 'KP' == $user->mahasiswa->jurusan ? 'selected' : '' }}>KP
                                    </option>
                                </select>
                            </div>
                            ...
                            <script>
                                function updateKelasOptions() {
                                    ...
                                    var defaultKelas = "<?php echo $user->Mahasiswa->kelas; ?>";
                                    var option = kelasSelect.querySelector("option[value='" + defaultKelas + "']");
                                    if (option) {
